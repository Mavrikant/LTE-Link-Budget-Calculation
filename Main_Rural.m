...
for i=1:Sample_size
    Pathloss_formula = A + B*log10((Distance(i)./1000)) - D; % Distance in Km; (Sample_size, 1) matrix
...
figure (5);
    ...
    grid on;
    title({'Coverage probability vs Distance (voice)';['RURAL, Hata Model, Fc=' num2str(Fc) 'Mhz, H-bts=' num2str(H_bts) 'm, Tx-power=' num2str(Tx_power) 'dB, Threshold-voice=' num2str(Threshold_voice) 'dB']});
    xlabel('Distance (m)');
    ylabel('Probability');
...
figure (6);
    ...
    axis([-inf +inf 0 1]) % fix y axis between 0 and 1
    grid on;
    title({'Coverage probability vs Distance (data)';['RURAL, Hata Model, Fc=' num2str(Fc) 'Mhz, H-bts=' num2str(H_bts) 'm, Tx-power=' num2str(Tx_power) 'dB, Threshold-data=' num2str(Threshold_data) 'dB']});
    xlabel('Distance (m)');
    ylabel('Probability'); 
