...
Threshold_voiceqpsk= -118 ; % UE connection Threshold_voice (Db)
Threshold_dataqpsk= -90 ; % UE connection Threshold_data (Db)
...
figure (1);
    ...
    title(['RURAL, HATA Model, Fc=' num2str(Fc) 'Mhz, H-bts=' num2str(H_bts) 'm, Tx-power=' num2str(Tx_power) 'dB, Threshold-voice=' num2str(Threshold_voiceqpsk) 'dB'])
    hold on;
    grid on;
    for i=1:UE_number
        if UE_database(i,5) %Data
            if UE_database(i,4) > Threshold_dataqpsk %Connected
                plot(UE_database(i,1), UE_database(i,2), '*g'); 
            else % NOT connected
                plot(UE_database(i,1), UE_database(i,2), '*r'); 
            end
        else %Voice
            if UE_database(i,4) > Threshold_voiceqpsk %Connected
                plot(UE_database(i,1), UE_database(i,2), '.g'); 
            else % NOT connected
                plot(UE_database(i,1), UE_database(i,2), '.r'); 
            end 
...
figure (2);
    Distance = linspace(0,Cell_size,2250); % 0-Cell_size Km, 1K sample
    plot(Distance, sort(Pathloss_formula)) % Distance in Km
    title({'Pathloss vs Distance (HATA)';['RURAL, HATA Model, Fc=' num2str(Fc) 'Mhz, H-bts=' num2str(H_bts) 'm, Tx-power=' num2str(Tx_power) 'dB, Threshold-voice=' num2str(Threshold_voiceqpsk) 'dB']})
...
%% Cumulative distribution of received power
    figure (3);
        ecdf(UE_database(:,4))
        title({'Cumulative distribution of received power';['RURAL, HATA Model, Fc=' num2str(Fc) 'Mhz, H-bts=' num2str(H_bts) 'm, Tx-power=' num2str(Tx_power) 'dB, Threshold-voice=' num2str(Threshold_voiceqpsk) 'dB']})
...
%% Histogram of received power by UE   
    figure (4);
        ...
        title({'Histogram of received power by UE';['RURAL, HATA Model, Fc=' num2str(Fc) 'Mhz, H-bts=' num2str(H_bts) 'm, Tx-power=' num2str(Tx_power) 'dB, Threshold-voice=' num2str(Threshold_voiceqpsk) 'dB']})
        xlabel('Received power (dB)');
        ylabel('Count');
        grid on;
...
for i=1:1000
    ...
    Coverage(i) = length(find(Power > Threshold_voiceqpsk))/Sample_size; % Find connected UE number
end
...
figure (5);
    ...
    title({'Coverage probability vs Distance';['RURAL, HATA Model, Fc=' num2str(Fc) 'Mhz, H-bts=' num2str(H_bts) 'm, Tx-power=' num2str(Tx_power) 'dB, Threshold-voice=' num2str(Threshold_voiceqpsk) 'dB']})
    xlabel('Distance (m)');
    ylabel('Probability');
    grid on;
...
for i=1:1000
    ...
    Shadowing_eff = normrnd (0, 8, [Sample_size, 1] ); % (Sample_size, 1) matrix
    Power = Tx_EiRP - Pathloss_formula - Shadowing_eff - Rx_body_loss ; % Received power by UE 
    Coverage(i) = length(find(Power > Threshold_dataqpsk))/Sample_size; % Find connected UE number
end
clear Power Pathloss_formula Shadowing_eff Sample_size
figure (6);
    plot (Distance, smooth(Coverage)); % Moving average smoothing
    axis([-inf +inf 0 1]); % fix y axis between 0 and 1
    title({'Coverage probability vs Distance';['RURAL, HATA Model, Fc=' num2str(Fc) 'Mhz, H-bts=' num2str(H_bts) 'm, Tx-power=' num2str(Tx_power) 'dB, Threshold-dataqpsk=' num2str(Threshold_voiceqpsk) 'dB']})
    xlabel('Distance (m)');
    ylabel('Probability');
    grid on;
    
Threshold_data64qam=-78;
Sample_size = 1000;       
Distance = linspace(0, Cell_size, Sample_size); % 0-15Km, 1K sample
...
for i=1:1000
    ...
    Shadowing_eff = normrnd (0, 8, [Sample_size, 1] ); % (Sample_size, 1) matrix
    Power = Tx_EiRP - Pathloss_formula - Shadowing_eff - Rx_body_loss ; % Received power by UE 
    Coverage1(i) = length(find(Power > Threshold_data64qam))/Sample_size; % Find connected UE number
end
clear Power Pathloss_formula Shadowing_eff Sample_size
figure (7);
    plot (Distance, smooth(Coverage1)); % Moving average smoothing
    axis([-inf +inf 0 1]); % fix y axis between 0 and 1
    title({'Coverage probability vs Distance';['RURAL, HATA Model, Fc=' num2str(Fc) 'Mhz, H-bts=' num2str(H_bts) 'm, Tx-power=' num2str(Tx_power) 'dB, Threshold-data 64QAM=' num2str(Threshold_data64qam) 'dB']})
    xlabel('Distance (m)');
    ylabel('Probability');
    grid on;
    
Fc2=1500;
A2 = 69.55 + 26.16*log10(Fc2) - 13.82*log10(H_bts) - ahm;
B2 = 44.9 - 6.55*log10(H_bts);
D2 = 4.78*log10(Fc2)^2 - 18.33*log10(Fc2) + 40.94;
Pathloss_formula2 = A2 + B2*log10(UE_database(:,3)/1000) - D2;  %(dB) (distance in Km)

figure (8);
    Distance = linspace(0,Cell_size,2250); % 0-Cell_size Km, 1K sample
    plot(Distance, sort(Pathloss_formula2)) % Distance in Km
    title({'Pathloss vs Distance (HATA)';['RURAL, HATA Model, Fc=' num2str(Fc2) 'Mhz, H-bts=' num2str(H_bts) 'm, Tx-power=' num2str(Tx_power) 'dB, Threshold-voice=' num2str(Threshold_voiceqpsk) 'dB']})
    xlabel('Distance in meter')
    ylabel('Pathloss in dB')
    clear Distance
    grid on
    
ahm2 = 3.2*(log10(1.75*H_ue))^2 - 4.97; % for large cities
A3 = 46.3 + 33.9*log10(Fc2) - 13.82*log10(H_bts) - ahm; 
B3 = 44.9 - 6.55*log10(H_bts);
C3 = 3; % 0 for medium-size city and suburban; 3 for metropolitan centers
Pathloss_formula3 = A3 + B3*log10(UE_database(:,3)/1000) + C3; % (dB) (distance in Km)

figure (9);
    Distance = linspace(0,Cell_size,2250); % 0-Cell_size Km, 1K sample
    plot(Distance, sort(Pathloss_formula3)) % Distance in Km
    title({'Pathloss vs Distance (COST 231)';['RURAL, COST 231 Model, Fc=' num2str(Fc2) 'Mhz, H-bts=' num2str(H_bts) 'm, Tx-power=' num2str(Tx_power) 'dB, Threshold-voice=' num2str(Threshold_voiceqpsk) 'dB']})
    xlabel('Distance in meter')
    ylabel('Pathloss in dB')
    clear Distance
    grid on
